  ...
  try {
    ...
    if (!token) {
      ...
      if (!token.startsWith('lip_')) {
        console.error(
          cl.red("Error: Invalid token format. Token must start with 'lip_'. Please check your token."),
        );
        readline.close();
        exit(1);
      }
      ...
      domain =
        (await question(cl.whiteBold('Lichess Domain (default: https://lichess.org): '))) ||
        'https://lichess.org';
