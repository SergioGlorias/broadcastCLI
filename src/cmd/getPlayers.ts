...
export const getPlayersCommand = async (args: string[]) => {
  ...
  const players = await client.GET('/broadcast/{broadcastTournamentId}/players', {
    params: {
  ...
  if (args.includes('--update')) {
    ...
    playersImportantInfo = await Promise.all(playersImportantInfo.map(async (player) => {
      try {
  ...
  if (style === 'table') {
    ...
    playersImportantInfo.forEach((player) => {
      console.log(`${player.name} / ${player.fideId} / ${player.title} / ${player.elo} / / ${player.country}`);
    });
  } else {
