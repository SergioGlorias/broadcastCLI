...
export const teamScoreCommand = async (args: string[]) => {
  ...
  if (!broadcastId || !teamWinStr || !teamDrawStr) {
    msgCommonErrorHelp(
      "Broadcast ID, team win, and team draw scores are required.",
    );
    exit(1);
  }
  // Parse scores
  const [teamWin, teamDraw] = [teamWinStr, teamDrawStr].map((scoreStr) =>
    parseFloat(scoreStr),
  );
  ...
  if ([teamWin, teamDraw].some(isNaN)) {
    msgCommonErrorHelp(
      "Scores for team win and team draw must be valid numbers.",
    );
    exit(1);
  }
  ...
  if ([teamWin, teamDraw].some((score) => score < 0 || score > 10)) {
    msgCommonErrorHelp(
      "Scores for team win and team draw must be between 0 and 10.",
    );
    exit(1);
  }
